...
class Setting_model extends CI_Model {
    ...
    public function update_profile() {
        ...
        $app_validity = $this->input->post('app_validity');
        $name = $this->db->where('type', 'app_validity')->get('tbl_settings')->row_array();
        if ($name) {
            $frm_name = ['message' => $app_validity];
            $this->db->where('type', 'app_validity')->update('tbl_settings', $frm_name);
        } else {
            $frm_name = array(
                'type' => 'app_validity',
                'message' => $app_validity
            );
            $this->db->insert('tbl_settings', $frm_name);
        }
    ...
    public function update_smtp_settings(){
        ...
        $pname = $this->db->where('type', 'protocol')->get('tbl_settings')->row_array();
        if ($pname) {
            $frm_name = ['message' => $protocol];
            $this->db->where('type', 'protocol')->update('tbl_settings', $frm_name);
        } else {
            $frm_name = array(
                'type' => 'protocol',
                'message' => $protocol
        ...
        $sname = $this->db->where('type', 'smtp_host')->get('tbl_settings')->row_array();
        if ($sname) {
            $frm_name = ['message' => $smtp_host];
            $this->db->where('type', 'smtp_host')->update('tbl_settings', $frm_name);
        } else {
            $frm_name = array(
                'type' => 'smtp_host',
                'message' => $smtp_host
        ...
        $pname = $this->db->where('type', 'smtp_port')->get('tbl_settings')->row_array();
        if ($pname) {
            $frm_name = ['message' => $smtp_port];
            $this->db->where('type', 'smtp_port')->update('tbl_settings', $frm_name);
        } else {
            $frm_name = array(
                'type' => 'smtp_port',
                'message' => $smtp_port
        ...
        $uname = $this->db->where('type', 'smtp_user')->get('tbl_settings')->row_array();
        if ($uname) {
            $frm_name = ['message' => $smtp_user];
            $this->db->where('type', 'smtp_user')->update('tbl_settings', $frm_name);
        } else {
            $frm_name = array(
                'type' => 'smtp_user',
                'message' => $smtp_user
        ...
        $pname = $this->db->where('type', 'smtp_pass')->get('tbl_settings')->row_array();
        if ($pname) {
            $frm_name = ['message' => $smtp_pass];
            $this->db->where('type', 'smtp_pass')->update('tbl_settings', $frm_name);
        } else {
            $frm_name = array(
                'type' => 'smtp_pass',
                'message' => $smtp_pass
    ...
    public function update_payment_settings(){
        ...
        $pname = $this->db->where('type', 'payment_mode')->get('tbl_settings')->row_array();
        if ($pname) {
            $frm_name = ['message' => $payment_mode];
            $this->db->where('type', 'payment_mode')->update('tbl_settings', $frm_name);
        } else {
            $frm_name = array(
                'type' => 'payment_mode',
        ...
        $sname = $this->db->where('type', 'api_key')->get('tbl_settings')->row_array();
        if ($sname) {
            $frm_name = ['message' => $api_key];
            $this->db->where('type', 'api_key')->update('tbl_settings', $frm_name);
        } else {
            $frm_name = array(
                'type' => 'api_key',
                'message' => $api_key
        ...
        $pname = $this->db->where('type', 'api_secret')->get('tbl_settings')->row_array();
        if ($pname) {
            $frm_name = ['message' => $api_secret];
            $this->db->where('type', 'api_secret')->update('tbl_settings', $frm_name);
        } else {
            $frm_name = array(
                'type' => 'api_secret',
                'message' => $api_secret
        ...
        $payment_gateway_title = $this->input->post('payment_gateway_title');
        $uname = $this->db->where('type', 'payment_gateway_title')->get('tbl_settings')->row_array();
        if ($uname) {
            $frm_name = ['message' => $payment_gateway_title];
            $this->db->where('type', 'payment_gateway_title')->update('tbl_settings', $frm_name);
        } else {
            $frm_name = array(
                'type' => 'payment_gateway_title',
                'message' => $payment_gateway_title
            );
            $this->db->insert('tbl_settings', $frm_name);
        }
        ...
        if ($_FILES['payment_logo']['name'] != '') {
            ...
            if ($this->upload->do_upload('payment_logo')) {
                ...
                $pname = $this->db->where('type', 'payment_logo')->get('tbl_settings')->row_array();
                if ($pname) {
                    $frm_name = ['message' => $img];
                    $this->db->where('type', 'payment_logo')->update('tbl_settings', $frm_name);
                } else {
                    $frm_name = array(
                        'type' => 'payment_logo',
                        'message' => $img
    ...
    public function update_coupon_data(){
        ...
        $this->db->where('id', $id)->update('tbl_coupon', $frm_data);
        return TRUE;
    }
