...
class Login extends CI_Controller {
    ...
    public function loginMe() {
        ...
        if ($result) {
            ...
            $app_validity = $this->db->where('type', 'app_validity')->get('tbl_settings')->row();
            $current_date = date('Y-m-d');
            if($result->status != "1"){
                if($current_date > $app_validity->message){
                    $this->session->set_flashdata('error', 'Your Licence expired. Contact your Service Provider ........ !');
                    redirect('', 'refresh');
                }
            }
