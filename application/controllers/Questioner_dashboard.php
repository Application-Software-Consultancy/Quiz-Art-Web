...
class Questioner_dashboard extends CI_Controller {
    ...
    public function index() {
        ...
        $this->result['today_question'] = $this->db->where('user_id', $qId)->where('created_at', date('Y-m-d'))->count_all_results('tbl_questioner_question');
        $this->result['weekly_question'] = $this->db->where('user_id', $qId)
                                            ->where('YEARWEEK(`created_at`, 1) = YEARWEEK(CURDATE(), 1)', NULL, FALSE)
                                            ->count_all_results('tbl_questioner_question');
        $this->result['monthly_question'] = $this->db->where('user_id', $qId)
                                             ->where('MONTH(`created_at`) = MONTH(CURDATE())', NULL, FALSE)
                                             ->where('YEAR(`created_at`) = YEAR(CURDATE())', NULL, FALSE)
                                             ->count_all_results('tbl_questioner_question');
        $this->result['yearly_question'] = $this->db->where('user_id', $qId)
                                         ->where('YEAR(`created_at`) = YEAR(CURDATE())', NULL, FALSE)
                                         ->count_all_results('tbl_questioner_question');
        
        $this->result['today_approved_question'] = $this->db->where('user_id', $qId)->where('status', '1')->where('created_at', date('Y-m-d'))->count_all_results('tbl_questioner_question');
        $this->result['weekly_approved_question'] = $this->db->where('user_id', $qId)->where('status', '1')
                                            ->where('YEARWEEK(`created_at`, 1) = YEARWEEK(CURDATE(), 1)', NULL, FALSE)
                                            ->count_all_results('tbl_questioner_question');
        $this->result['monthly_approved_question'] = $this->db->where('user_id', $qId)->where('status', '1')
                                             ->where('MONTH(`created_at`) = MONTH(CURDATE())', NULL, FALSE)
                                             ->where('YEAR(`created_at`) = YEAR(CURDATE())', NULL, FALSE)
                                             ->count_all_results('tbl_questioner_question');
        $this->result['yearly_approved_question'] = $this->db->where('user_id', $qId)->where('status', '1')
                                         ->where('YEAR(`created_at`) = YEAR(CURDATE())', NULL, FALSE)
                                         ->count_all_results('tbl_questioner_question');
        ...
        $this->load->view('questioner_dashboard', $this->result);
    }
    ...
    public function create_question(){
            if ($this->input->post('btnadd')) {
                $data = $this->Questioner_model->add_question_data();
                $this->session->set_flashdata('success', 'Question created successfully.! ');
                redirect('create-question', 'refresh');
            }
            if ($this->input->post('btnupdate')) {
            ...
            $this->load->view('create_questions', $this->result);
    }
    
    public function question_list(){
        $this->result['language'] = $this->Language_model->get_data();
        $this->result['category'] = $this->Chapter_model->get_category_data();
        $this->load->view('all_question_list', $this->result);
    }
    
    public function edit_questioner_questions($id){
        if ($this->input->post('btnupdate')) {
            $this->Questioner_model->add_question_all_data();
            $this->session->set_flashdata('success', 'Question updated successfully.!');
            redirect('list-question', 'refresh');
        }
        $this->result['category'] = $this->Chapter_model->get_category_data();
        $this->result['question_data'] = $this->Questioner_model->get_all_data($id);
        $this->load->view('questions_edit', $this->result);
    }
